          ...
          def analyzeCorrelationWithPrice(df: DataFrame,targetCol: String = "price",excludeCols: Seq[String] = Seq() ): Unit = {
            ...
            results.foreach { case (colName, v) =>
              ...
              println(f"| $colName%-30s | ${v}%+10.4f | $label%-6s |")
            }
          ...
          //esta función calcula las estadísticas de una variable numérica 
          // a nivel global, incluyendo medidas de tendencia central, dispersión y forma de la distribución, 
          ...
          prefix: String = ""): DataFrame = {
            ...
            val sfx = if (prefix.isEmpty) "" else s"_$prefix"

              getValidNumericDF(df, numericCol, positiveOnly).select(
              count("*")                                        .alias(s"n$sfx"),
              min(numericCol)                                   .alias(s"min$sfx"),
              max(numericCol)                                   .alias(s"max$sfx"),
              avg(numericCol)                                   .alias(s"mean$sfx"),
              expr(s"percentile_approx($numericCol, 0.5)")      .alias(s"median$sfx"),
              stddev(numericCol)                                .alias(s"std$sfx"),
              skewness(numericCol)                              .alias(s"skew$sfx"),
              kurtosis(numericCol)                              .alias(s"kurt$sfx")
              )
          }
          ...
          prefix: String = "",probs: Seq[Double] = Seq(0.01, 0.05, 0.25, 0.50, 0.75, 0.90, 0.95, 0.99)): DataFrame = {

            val colName = if (prefix.isEmpty) numericCol else prefix

            val percentileValues = getValidNumericDF(df, numericCol, positiveOnly).stat.approxQuantile(numericCol, probs.toArray, 0.001)

            probs.zip(percentileValues).toDF("percentile", s"value_$colName")
          }
          ...
          def analyzeNumericVsLog(df: DataFrame,numericCol: String,positiveOnly: Boolean = true,prefix: String = ""): DataFrame = {

            val sfx = if (prefix.isEmpty) "" else s"_$prefix"
            getValidNumericDF(df, numericCol, positiveOnly).withColumn("log_value_tmp", log1p(col(numericCol))).select(
                skewness(numericCol)       .alias(s"skew$sfx"),
                kurtosis(numericCol)       .alias(s"kurt$sfx"),
                skewness("log_value_tmp")  .alias(s"skew_log$sfx"),
                kurtosis("log_value_tmp")  .alias(s"kurt_log$sfx")
              )
          }
          ...
          def getTopExtremeValues(df: DataFrame,numericCol: String,positiveOnly: Boolean = false,topN: Int = 30,
          extraCols: Seq[String] = Seq()): DataFrame = {
          ...
          def getNumericSegmentStats(df: DataFrame,numericCol: String,segmentCols: Seq[String],positiveOnly: Boolean = false,
          minGroupSize: Int = 30,prefix: String = ""): DataFrame = {

            val sfx = if (prefix.isEmpty) "" else s"_$prefix"
            val q1Col = s"q1$sfx" 
            val q3Col = s"q3$sfx"

            val stats = getValidNumericDF(df, numericCol, positiveOnly).groupBy(segmentCols.filter(df.columns.contains).map(col): _*).agg(
                count("*")                                          .alias("group_n"),
                min(numericCol)                                     .alias(s"min$sfx"),
                expr(s"percentile_approx($numericCol, 0.25)")       .alias(q1Col),
                expr(s"percentile_approx($numericCol, 0.5)")        .alias(s"median$sfx"),
                expr(s"percentile_approx($numericCol, 0.75)")       .alias(q3Col),
                expr(s"percentile_approx($numericCol, 0.95)")       .alias(s"p95$sfx"),
                expr(s"percentile_approx($numericCol, 0.99)")       .alias(s"p99$sfx"),
                avg(numericCol)                                     .alias(s"mean$sfx"),
                max(numericCol)                                     .alias(s"max$sfx"),
                stddev(numericCol)                                  .alias(s"std$sfx"),
                skewness(numericCol)                                .alias(s"skew$sfx")
              ).filter(col("group_n") >= minGroupSize).withColumn(s"cv_pct$sfx", (col(s"std$sfx") / col(s"mean$sfx")) * 100)
              
              val withBounds = addIQRBounds(stats, q1Col = q1Col, q3Col = q3Col).withColumn("lower_bound_iqr_1_5",
              when(col("lower_bound_iqr_1_5") < 0, 0).otherwise(col("lower_bound_iqr_1_5")))
              
              withBounds.columns.foldLeft(withBounds) { (acc, colName) =>acc.schema(colName).dataType match {
                  case org.apache.spark.sql.types.DoubleType =>
                  acc.withColumn(colName, round(col(colName), 2))
                  case _ => acc
                }
            }
          }
          ...
          //esta funcion muestra las estadisticas en dos tablas, 
          //una con las medidas de tendencia central y otra con las medidas de dispersión y límites de outliers (IQR), 
          //para un segmento específico definido por una columna categórica.
          def showSegmentStats(title: String, df: DataFrame, segmentCol: String, n: Int = 20): Unit = {

            val line = "═" * (title.length + 6)
            println(s"\n  $line")
            println(s"  ║  $title  ║")
            println(s"  $line")

            
            val centrocols = Seq(segmentCol, "group_n") ++
              df.columns.filter(c =>
                c.startsWith("min")    || c.startsWith("q1")     ||
                c.startsWith("median") || c.startsWith("q3")     ||
                c.startsWith("p95")    || c.startsWith("p99")    ||
                c.startsWith("max")    || c.startsWith("mean")
              ).toSeq

            println(s"\n  [1/2] Tendencia central")
            df.select(centrocols.filter(df.columns.contains).map(col): _*).show(n, truncate = false)

            // ── Tabla 2: dispersión y outliers ────────────────────────
            val dispersioCols = Seq(segmentCol, "group_n") ++
              df.columns.filter(c =>
                c.startsWith("std")   || c.startsWith("skew")  ||
                c.startsWith("cv_pct")|| c == "iqr"            ||
                c.startsWith("lower") || c.startsWith("upper")
              ).toSeq

            println(s"\n  [2/2] Dispersión y límites de outliers (IQR)")
            df.select(dispersioCols.filter(df.columns.contains).map(col): _*).show(n, truncate = false)
          }
          ...
          def analyzePriceByTopCategories(df: DataFrame,categoryCol: String,priceCol: String = "price",topCategories: Int = 20): DataFrame = {

              val topCats = df.groupBy(categoryCol).count().orderBy(desc("count")).limit(topCategories).select(categoryCol)

              getNumericSegmentStats(
                df           = df.join(topCats, Seq(categoryCol)),
                numericCol   = priceCol,
                segmentCols  = Seq(categoryCol),
                positiveOnly = false,
                minGroupSize = 1,
                prefix       = priceCol
              ).orderBy(desc(s"median_$priceCol"))
          }
           
          def getTopExpensiveVehicles(df: DataFrame,priceCol: String = "price",topN: Int = 30): DataFrame = {
            getTopExtremeValues(df = df,numericCol = priceCol,positiveOnly = true,topN = topN,
              extraCols = Seq("make_name", "model_name", "trim_name", "year",
               "body_type", "fuel_type", "is_new", "mileage", "horsepower")
            )
          }

          def getTopKPriceByCategory( df: DataFrame,categoryCol: String,priceCol: String = "price",topCategories: Int = 15,k: Int = 5): DataFrame = {
            val dfPrice = getValidNumericDF(df, priceCol, positiveOnly = true)
          ...
          def detectSuspiciousPriceOutliersHierarchical(df: DataFrame,priceCol: String = "price",minGroupSize: Int = 30,
              iqrMultiplier: Double = 3.0,minAbsolutePrice: Double = 100000.0): DataFrame = {
          ...
          def summarizeSuspiciousPriceOutliers(suspiciousDF: DataFrame,segmentCols: Seq[String]): DataFrame = {
          ...
          def edaAddGeoRegion(   df: DataFrame,    latitudeCol: String = "latitude",    
          ...
          def analyzeTwoNumericVars(df: DataFrame,col1: String,col2: String,positiveOnly: Boolean = true,
          prefix1: String = "",prefix2: String = ""): DataFrame = {

            val p1 = if (prefix1.isEmpty) col1 else prefix1
            val p2 = if (prefix2.isEmpty) col2 else prefix2

            val dfValid = df.filter(col(col1).isNotNull && col(col2).isNotNull)
            .filter(if (positiveOnly) col(col1) > 0 && col(col2) > 0 else lit(true))
            .withColumn("ratio_tmp", round(col(col2) / col(col1) * 100, 2))

            dfValid.select(
              count("*")                                                    .alias("n"),
              round(corr(col1, col2),                              4)       .alias(s"correlacion_${p1}_${p2}"),
              round(avg("ratio_tmp"),                              2)       .alias(s"ratio_pct_medio"),
              round(expr("percentile_approx(ratio_tmp, 0.5)"),     2)       .alias(s"ratio_pct_mediana"),
              round(min("ratio_tmp"),                              2)       .alias(s"ratio_pct_min"),
              round(max("ratio_tmp"),                              2)       .alias(s"ratio_pct_max"),
              round(avg(col2),                                     2)       .alias(s"mean_$p2"),
              round(expr(s"percentile_approx($col2, 0.5)"),        2)       .alias(s"median_$p2"),
              round(min(col2),                                     2)       .alias(s"min_$p2"),
              round(max(col2),                                     2)       .alias(s"max_$p2"),
              round(avg(col1),                                     2)       .alias(s"mean_$p1"),
              round(expr(s"percentile_approx($col1, 0.5)"),        2)       .alias(s"median_$p1"),
              round(min(col1),                                     2)       .alias(s"min_$p1"),
              round(max(col1),                                     2)       .alias(s"max_$p1")
            )
          }
          ...
          def extractTopOptions(df: DataFrame,optionsCol: String = "major_options",topN: Int = 20,minFreqPct: Double = 1.0,
          sampleFraction: Double = 0.2,seed: Long = 42L,
          excludeTerms: Seq[String] = Seq("quickorderpackage", "steelwheels", "conveniencepackage","premiumpackage", 
          "towpackage", "appearancepackage","powerpackage", "technologypackage", "preferredpackage","coldweatherpackage", 
          "heatpackage", "sepackage","premiumwheels", "chromewheels")
          ): DataFrame = {

            val dfSample = df.filter(col(optionsCol).isNotNull).sample(withReplacement = false, sampleFraction, seed).select(optionsCol).cache()

            val totalSample = dfSample.count()

            val result = dfSample.withColumn("option_raw", explode(split(
              regexp_replace(lower(col(optionsCol)), "[\\[\\]'\"\\s]", ""), ",")))
              .filter(col("option_raw") =!= "" && length(col("option_raw")) > 3)
              .filter(!col("option_raw").isin(excludeTerms: _*))  // ← filtro temprano
              .groupBy("option_raw")
              .agg(count("*").alias("frecuencia"))
              .withColumn("pct", round(col("frecuencia") / totalSample * 100, 2))
              .filter(col("pct") >= minFreqPct)
          ...
          /*
            ...
            showDF("Comparación price vs log(price)",
            analyzeNumericVsLog(df,numericCol = "price",positiveOnly = true,prefix = "price"))


             
            showDF("Relación entre precio y descuento",
            analyzeTwoNumericVars(df, col1 = "price", col2 = "savings_amount",prefix1 = "price", prefix2 = "savings"))

            showDF("Relación entre precio y kilometraje",
            analyzeTwoNumericVars(df, col1 = "price", col2 = "mileage",prefix1 = "price", prefix2 = "mileage"))
            
            showDF("Relación entre precio y potencia",
            analyzeTwoNumericVars(df, col1 = "price", col2 = "horsepower",prefix1 = "price", prefix2 = "hp"))
            
           
              val df2 = df.withColumn("vehicle_age_at_listing",year(col("listed_date")) - col("year"))

            showDF("Relación entre precio y antigüedad",
            analyzeTwoNumericVars(df2,col1= "price",col2 = "vehicle_age_at_listing",prefix1 = "price",
            prefix2 = "age",positiveOnly = false))
           ...
           println("ANALISIS DE DISTRIBUCION:")

            showDF("Distribución de precio en las 10 marcas más frecuentes", 
            analyzePriceByTopCategories(df, "make_name", topCategories = 10))


            showSegmentStats(title      = "Distribución de precio: vehículos nuevos vs usados",
            df= getNumericSegmentStats(df= df,numericCol   = "price",segmentCols  = Seq("is_new"),positiveOnly = true,
            minGroupSize = 1,prefix       = "price").orderBy(desc("median_price")),segmentCol = "is_new",n= 10)

            showSegmentStats(title      = "Distribución de precio por tipo de carrocería",
            df= getNumericSegmentStats(df= df,numericCol   = "price",segmentCols  = Seq("body_type"),positiveOnly = true,
            minGroupSize = 30,prefix       = "price").orderBy(desc("median_price")),segmentCol = "body_type",n= 20)  


            showDF("Distribución de precio por tipo de combustible — Top 10 más frecuentes",
            analyzePriceByTopCategories(df, "fuel_type", topCategories = 10)) 

            showSegmentStats(title      = "Evolución del precio por año de fabricación — últimos 20 años",
            df= getNumericSegmentStats(df= df,numericCol   = "price",segmentCols  = Seq("year"),positiveOnly = true,
            minGroupSize = 1,prefix       = "price").orderBy(desc("year")),segmentCol = "year",n= 20)
            ...
            val dfGeo = addGeographicFeatures(df)


            showSegmentStats(  "Distribución de precio por región geográfica",
            df= getNumericSegmentStats(df= dfGeo,numericCol   = "price",segmentCols  = Seq("geo_region"),positiveOnly = true,
            prefix       = "price").orderBy(desc("median_price")),segmentCol = "geo_region",n= 10)




            val dfUrbanLevel = dfGeo.filter(col("urban_level").isNotNull)

            showSegmentStats("Distribución de precio por proximidad urbana",
            df= getNumericSegmentStats(df= dfUrbanLevel,numericCol   = "price",segmentCols  = Seq("urban_level"),positiveOnly = true,
            prefix= "price").orderBy(when(col("urban_level") === "urban",    1).when(col("urban_level") === "suburban", 2).otherwise(3)),
            segmentCol = "urban_level",n= 10)
            ...
            showSegmentStats(title      = "Distribución de kilometraje: vehículos nuevos vs usados",
            df= getNumericSegmentStats(df           = df,numericCol   = "mileage",segmentCols  = Seq("is_new"),
            minGroupSize = 1,prefix       = "mileage").orderBy(desc("median_mileage")),segmentCol = "is_new",n= 10)

            println("ANALISIS DE LA VARIABLE DAYSONMARKET:")  

             
            showSegmentStats(title = "Días en mercado por tipo de carrocería",
            df = getNumericSegmentStats(df = df,numericCol   = "daysonmarket",segmentCols  = Seq("body_type"),minGroupSize = 30,
            prefix       = "dom").orderBy(desc("median_dom")),segmentCol = "body_type",n = 20)
           ...
           /*
            val topOptions = extractTopOptions(df, topN = 30, minFreqPct = 1.0)
            showDF("Opciones más frecuentes en major_options", topOptions, n = 30)
            
            val topOptionsFiltradas = topOptions.filter(!col("option_raw").isin(opcionesExcluir: _*))

            showDF("Opciones relevantes seleccionadas", topOptionsFiltradas, n = 20)
          */
            val dfOptions = addMajorOptionsFeaturesFromData(df)
            val optionCols = dfOptions.columns.filter(_.startsWith("has_")).toSeq


            println("\n  Frecuencia de opciones en el dataset")
            println("  " + "─" * 55)
            val total = dfOptions.count()
            optionCols.foreach { optCol =>
              val con = dfOptions.filter(col(optCol) === 1).count()
              val pct = con.toDouble / total * 100
              println(f"  $optCol%-35s → $con%,7d ($pct%5.1f%%)")
            }

          
            optionCols.foreach { optCol =>
              val titulo = s"Impacto en precio: ${optCol.replace("has_", "").replace("_", " ")}"
              showSegmentStats(
                title      = titulo,
                df         = getNumericSegmentStats(
                              df           = dfOptions,
                              numericCol   = "price",
                              segmentCols  = Seq(optCol),
                              positiveOnly = true,
                              minGroupSize = 30,
                              prefix       = "price"
                            ).orderBy(asc(optCol)),
            ...
            showSegmentStats(
              title      = "Impacto de la densidad de equipamiento en el precio",
              df         = getNumericSegmentStats(
                            df           = dfOptions,
                            numericCol   = "price",
                            segmentCols  = Seq("option_count"),
                            positiveOnly = true,
                            minGroupSize = 30,
                            prefix       = "price"
                          ).orderBy(asc("option_count")),
              segmentCol = "option_count",
              n          = 20
            )
 ...
 */
            ...
            validarParesRedundantes(df, candidatos)
             ...
             val dfgeo = addGeographicFeatures(df) 
            showSegmentStats(title = "Distribución de precio por nivel urbano",
            df = getNumericSegmentStats(df          = dfgeo, numericCol  = "price",segmentCols = Seq("urban_level"),
            minGroupSize = 30,prefix      = "price").orderBy(desc("median_price")),segmentCol = "urban_level",n= 20)
            
            showSegmentStats(title= "Días en mercado por nivel urbano",
            df = getNumericSegmentStats(df= dfgeo,numericCol  = "daysonmarket",segmentCols = Seq("urban_level"),
            minGroupSize = 30,prefix      = "dom").orderBy(desc("median_dom")),segmentCol = "urban_level",n= 20)

             
            val corrDist = dfgeo.select(corr(col("dist_to_major_city_miles"), col("price"))).collect()(0).getDouble(0)
            println(f"  Correlación dist_to_major_city_miles ↔ price: $corrDist%.4f")
